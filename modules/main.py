        ...
        with t_view:
            try:
                ...
                if res.data:
                    ...
                    u_sel = st.selectbox("👤 Select User to Edit", sorted(df_u['username'].tolist()))
                    u_data = df_u[df_u['username'] == u_sel].iloc[0]
                    with st.form("edit_user"):
                        e_pass = st.text_input("Password", value=u_data['password'])
                        e_role = st.selectbox("Role", role_options, index=role_options.index(u_data['role']) if u_data['role'] in role_options else 0)
                        if st.form_submit_button("💾 Save Changes"):
                            supabase.table("users").update({"password": e_pass, "role": e_role}).eq("username", u_sel).execute()
                            st.success("Updated!")
                            st.rerun()
            except: pass
