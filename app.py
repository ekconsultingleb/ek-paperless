...
# --- 2. THE APP DASHBOARD ---
    ...
    if st.session_state['module'] == "daily_cash":
        st.markdown(f"## 🏦 Daily Cash Report: {st.session_state['user'].capitalize()}")
        
        try:
            df_cash = conn.read(spreadsheet=st.session_state['link'], worksheet="cash", ttl=0)
    ...
    # ==========================================
        ...
        st.markdown(f"## 📦 Inventory Count: {st.session_state['user'].capitalize()}")
        ...
        try:
            ...
            with st.form("mobile_inventory_form"):
                st.warning("⚠️ **IMPORTANT:** You MUST click the 'Save All Changes' button below BEFORE changing filters!")
                new_quantities = {}
                    ...
                    if st.form_submit_button("Add Item to Database"):
                        if new_desc and new_unit:
                            ...
                            if new_desc.lower().strip() in existing_items:
                                st.error(f"🛑 Hold on! '{new_desc}' is already in the database. Use the Search bar!")
                            else:
    ...
    # ==========================================
        ...
        try:
            ...
            if declaration == "🎉 Event / Function":
                ...
                if not event_name:
                    st.warning("⚠️ Please type the Event Name before saving!")
            ...
            stat_filter = st.sidebar.selectbox("Status", statuses)
            ...
            # --- 3. MOBILE TICKET ENTRY ---
            st.write(f"### 📝 Enter Quantities for: **{declaration.split(' ')[1]}**")
            ...
            with st.form("waste_ticket_form",clear_on_submit=True):
                ...
                for index, row in filtered_df.iterrows():
                    with st.container(border=True):
                        ...
                        with col2:
                            new_quantities[index] = st.number_input(
                                "Qty", value=0.0, min_value=0.0, step=1.0,
                                key=f"w_qty_{index}", label_visibility="collapsed" 
                            )
                        
                        new_remarks[index] = st.text_input(
                            "Remark", value="", key=f"w_rem_{index}",
                            placeholder="Optional remark...", label_visibility="collapsed"
                        )
                            
                submit_button = st.form_submit_button("🚀 Submit Ticket to Archive", type="primary", use_container_width=True)
                
                if submit_button:
                    if declaration == "🎉 Event / Function" and not event_name:
                        st.error("🛑 Cannot submit! Please scroll up and enter the Event Name.")
                    else:
                        ticket_items = []
                        ...
                        for idx, row in filtered_df.iterrows():
                            ...
                            if qty > 0:
                                # --- THE AUTO-CALC BRAIN ---
                                cat_text = str(row['Category']).lower()
                                is_bev = 'bev' in cat_text or 'drink' in cat_text or 'bar' in cat_text
                                
                                if declaration == "🗑️ Daily Waste":
                                    tag = "[wb]" if is_bev else "[wf]"
                                elif declaration == "🍽️ Staff Meal":
                                    tag = "[smb]" if is_bev else "[sm]" 
                                elif declaration == "🎉 Event / Function":
                                    tag = f"[theo b - {event_name}]" if is_bev else f"[theo f - {event_name}]"
                                
                                final_remark = f"{tag} {new_remarks[idx]}".strip()
                                
                                new_row = row.copy()
                                new_row['Qty'] = qty
                                new_row['Remarks'] = final_remark
                                new_row['Archive Date'] = datetime.today().strftime("%Y-%m-%d")
                        ...
                        if ticket_items:
                            ...
                            st.success(f"✅ {declaration} successfully logged to Data Entry!")
                        else:
                            st.warning("You didn't enter any quantities! Nothing was saved.")
    ...
    # ==========================================
        ...
        st.markdown("## 👑 Master Admin Dashboard")
